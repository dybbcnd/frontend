...
export default function AdminDashboard() {
    return (
        <main className="min-h-screen p-8 bg-gray-900 bg-opacity-80"
            style={{
                backgroundImage: "url('./dshbrd-bg.jpg')",
            ...
            <section className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                {stats.map((stat) => (
                    <div key={stat.label} className="bg-gray-600 opacity-75 rounded shadow p-6 text-center">
                        <div className="text-2xl font-semibold">{stat.value}</div>
                        <div className="text-gray-100">{stat.label}</div>
                    </div>
                ))}
            </section>
            <section className="bg-gray-600 opacity-75 rounded shadow p-6">
